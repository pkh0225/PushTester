...
- **Payload 도구**
  - **커스텀 템플릿**: 현재 Payload를 이름과 함께 저장·불러오기·관리 (iOS/Android 분리)
  - **JSON 유효성 표시**: Payload 옆에서 실시간 검사, 전송 전에도 재검증
...
- **다중 토큰 전송**
  - Device Token 프리셋에 저장된 토큰을 개별/전체로 연속 전송
  - Push 버튼 옆 ▾ 메뉴에서 선택
  - 결과 요약(`Success n/m`)과 토큰별 히스토리 기록
- **필드 프리셋**
...
- **전송 히스토리** (왼쪽 패널)
  - 전송 기록(성공·HTTP 실패 포함), 적용 / 편집 / 삭제
  - **응답 상세**: APNs/FCM 응답 헤더·바디 보기·복사
  - 현재 플랫폼 히스토리 **전체 삭제**
  - iOS / Android 탭별 목록 분리
- **설정** (타이틀바 가운데 `PushTester` 옆 ⚙)
  - **앱 정보** (이름, 버전, 플랫폼, GitHub 링크, 간단한 설명)
  - 설정 목록 UI (`AppSettingsCatalog`로 항목 확장 가능)
  - **전체 데이터 초기화** (히스토리·저장목록·프리셋·인증서·Payload 템플릿·마지막 세션·플랫폼 탭·입력 폼)
- 마지막 세션 자동 저장/복원
...
Payload 영역 헤더에 템플릿·변환·전송 컨트롤이 있습니다.

1. **템플릿** 메뉴로 저장해 둔 Payload를 불러오고, **저장**으로 현재 내용을 이름으로 남깁니다. ⚙에서 목록을 관리합니다.
2. 옆의 **JSON 유효 / JSON 오류**로 전송 전 상태를 확인합니다. 오류면 Push가 바로 막히고 상태바에 이유가 표시됩니다.
3. **Key 선택**에서 변환할 JSON key 경로를 고릅니다.
4. **String 치환**: int/bool/object/array 등을 문자열로 바꿉니다. object/array는 compact JSON 문자열이 됩니다.
5. 같은 버튼을 다시 누르면 **String 원복**으로 이전 타입을 되돌립니다.
6. **인코딩**: 선택한 value를 URL percent-encoding 합니다. (object/array는 JSON 텍스트로 만든 뒤 인코딩)
7. 다시 누르면 **인코딩 원복**입니다.
8. Payload 편집 중에는 macOS가 `"` 를 스마트 따옴표로 바꾸지 않습니다. 이미 섞여 있는 경우 세션 복원·Push 전송 시 ASCII로 교정됩니다.

### 다중 토큰 전송

1. Device Token 라벨 ⚙으로 여러 토큰을 프리셋에 저장합니다.
2. Push 버튼 오른쪽 **▾** 을 엽니다.
3. 개별 프리셋 토큰으로 보내거나 **프리셋 토큰 전체 전송**을 선택합니다.
4. 각 토큰 전송 결과는 히스토리에 남고, 상태바에 성공/실패 요약이 표시됩니다.
...
1. 푸시를 보내면 왼쪽 목록에 기록됩니다 (HTTP 실패 응답도 포함).
2. 항목을 선택한 뒤 **적용**으로 폼에 반영합니다.
3. 항목을 우클릭한 뒤 **응답 상세**로 APNs/FCM 응답 헤더·바디를 확인하고 복사할 수 있습니다.
4. **편집** / **삭제**(단일) / 헤더 **휴지통**(현재 플랫폼 전체 삭제)을 사용할 수 있습니다.
5. 툴바 왼쪽 버튼으로 패널을 접거나 펼칠 수 있습니다.
6. iOS / Android 탭에 따라 히스토리가 분리됩니다.
7. 히스토리 전체 삭제는 저장목록·프리셋 등 다른 데이터에는 영향을 주지 않습니다.
...
~/Library/Application Support/PushTester/
  ...
  payload-templates.json     # 커스텀 Payload 템플릿
